...
class PostgresCache {
	get cache() {
		const cacheDelegate = prisma?.cacheEntry;
		if (!cacheDelegate) {
			throw new Error('Prisma model "CacheEntry" is not available. Please run `bunx prisma generate` and ensure the schema is pushed (`bunx prisma db push`).');
		}
	...
	async keys(prefix = '') {
		const entries = await this.cache.findMany({
			where: prefix ? { key: { startsWith: prefix } } : undefined,
			select: { key: true },
		});
	...
	async exists(key) {
		return this.cache.count({ where: { key } });
	}

	async expire(key, ttlSeconds) {
		const expiresAt = buildExpiresAt(ttlSeconds || 0);
		const entry = await this.cache.updateMany({
			where: { key },
			data: { expiresAt },
		});
		return entry.count;
	...
	async findFreshEntry(key) {
		const entry = await this.cache.findUnique({ where: { key } });
		if (isExpired(entry)) {
			await this.cache.delete({ where: { key } }).catch(() => {});
			return null;
		}
		return entry;
	}
